<div>
    <x-atoms.breadcrumb currentPath="Hero SPMB" />
    ...
    <x-atoms.card className="mt-3">
        <div class="flex justify-between items-center mb-6">
            ...
            <x-atoms.button
                ...
                theme="light"
                heroicon="pencil">
                {{ $heroSpmb ? 'Edit Gambar' : 'Upload Gambar' }}
            </x-atoms.button>
        </div>

        @if($heroSpmb && $heroSpmb->image)
        <div class="space-y-6">
            {{-- Gambar --}}
            <div class="relative">
                <img src="{{ asset('storage/' . $heroSpmb->image) }}"
                    alt="Hero SPMB"
            ...
            <div class="flex items-center justify-between text-sm text-gray-500 pt-4 border-t border-gray-200">
                <div class="flex items-center gap-4">
                    <span class="flex items-center">
                        ...
                        Dibuat: {{ $heroSpmb->created_at->format('d M Y H:i') }}
                    </span>
                    @if($heroSpmb->updated_at != $heroSpmb->created_at)
                    <span class="flex items-center">
                        <x-heroicon-o-arrow-path class="w-4 h-4 mr-1" />
                        Diperbarui: {{ $heroSpmb->updated_at->format('d M Y H:i') }}
                    </span>
                    @endif
                </div>
            </div>
        ...
        <div class="text-center py-12">
            ...
            <x-atoms.title text="Belum Ada Gambar Hero SPMB" size="lg" class="text-gray-900 mb-2" align="center" />
            <x-atoms.description class="text-gray-600 mb-6">
                Upload gambar untuk ditampilkan di halaman Hero SPMB.
            </x-atoms.description>
            <center>
                <x-atoms.button
                    wire:click="edit"
    ...
    {{-- Modal --}}
    <x-atoms.modal name="hero-spmb-modal" maxWidth="lg" :closeable="true">
        <div class="flex justify-between items-center p-6 border-b border-gray-200">
            <x-atoms.title
                :text="$heroSpmb ? 'Edit Gambar Hero SPMB' : 'Upload Gambar Hero SPMB'"
                size="lg"
                class="text-gray-900" />
        </div>
        ...
        <div class="p-6">
            <form wire:submit.prevent="save" class="space-y-6">
                {{-- Upload gambar --}}
                <div class="space-y-3">
                    <x-atoms.label for="new_image">Gambar Hero SPMB</x-atoms.label>
        ...
        <div class="flex gap-3 p-6 border-t border-gray-200">
            <x-atoms.button
                ...
                heroicon="check"
                isFullWidth>
                {{ $heroSpmb ? 'Update Gambar' : 'Simpan Gambar' }}
            </x-atoms.button>
            <x-atoms.button
                wire:click="cancel"
                variant="secondary"
