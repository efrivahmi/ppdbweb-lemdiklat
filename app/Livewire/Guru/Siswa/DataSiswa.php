...
class DataSiswa extends Component
{
    ...
    public function deleteSiswa($id)
    {
        try {
            ...
            if ($siswa->role !== 'siswa') {
                $this->dispatch("alert", message: "Hanya dapat menghapus siswa", type: "warning");
                return;
            }
    ...
    public function checkNISN()
    {
        if ($this->nisn) {
            ...
            if ($exists) {
                $this->dispatch("alert", message: "Nisn sudah digunakan", type: "warning");
            } else {
    ...
    public function render()
    {
        $siswas = User::where('role', 'siswa')
            ->with(['dataMurid', 'berkasMurid', 'buktiTransfer', 'pendaftaranMurids'])
            ...
            ->when($this->statusFilter, function ($query) {
                switch ($this->statusFilter) {
                    case 'data_lengkap':
                        ...
                        break;
                    case 'berkas_lengkap':
                        $query->whereHas('berkasMurid', fn($q) => $q->where('proses', 1));
                        break;
        ...
        return view('livewire.guru.siswa.data-siswa', ['siswas' => $siswas]);
    }
}